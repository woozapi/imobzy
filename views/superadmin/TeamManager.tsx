...
import { 
  ...
  Trash2, Search, MoreHorizontal, UserCheck, ShieldAlert
} from 'lucide-react';
...
const TeamManager: React.FC = () => {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Funcionalidade de convite enviada para ${inviteEmail}. O usuário receberá um link para configurar a senha.`);
  ...
  const removeStaff = async (id: string) => {
    if (!confirm('Deseja remover o acesso de Super Admin deste usuário?')) return;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gerencie os administradores e agentes de suporte internos da IMOBZY.</p>
        </div>
        <button 
          ...
          <table className="w-full">
            <thead className="bg-gray-50 text-left">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Data de Início</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-400">Carregando membros...</td></tr>
              ) : filteredStaff.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-400">Nenhum membro da equipe encontrado.</td></tr>
              ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl p-6 text-white">
          ...
          <p className="text-indigo-100 text-sm leading-relaxed">Todos os membros da equipe Super Admin possuem acesso total à governança da plataforma. Use com cuidado.</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <Mail className="text-indigo-600 mb-4" size={32} />
          <h3 className="font-bold text-lg mb-2">Central de Convites</h3>
          <p className="text-gray-500 text-sm leading-relaxed">Você pode convidar novos membros via e-mail corporativo para se juntarem à equipe de suporte.</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <Calendar className="text-indigo-600 mb-4" size={32} />
          <h3 className="font-bold text-lg mb-2">Auditoria de Log</h3>
          <p className="text-gray-500 text-sm leading-relaxed">Toda ação realizada por membros da equipe é registrada no log de auditoria global.</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-900">Convidar para Equipe</h3>
              <button onClick={() => setIsInviteModalOpen(false)} className="text-gray-400 hover:text-gray-600 text-xs font-bold uppercase">Fechar</button>
            </div>
            ...
            <form onSubmit={handleInvite} className="p-6 space-y-4">
              ...
              <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                <p className="text-xs text-indigo-700 leading-relaxed font-medium">
                  <strong>Aviso:</strong> O convidado receberá um e-mail com instruções para ativar sua conta administrativa.
                </p>
              </div>
